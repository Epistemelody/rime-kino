...
def gen_commands(draft: Path, overlay: Path) -> None:
    ...
    parts = ["-- generated by scripts/gen_overlay.py; do not edit\n", "local rows = {\n"]
    for commit, code, kind in packed:
        parts.append(f"  {{{lua_str(commit)}, {lua_str(code)}, {lua_str(kind)}}},\n")
    parts.append("}\nlocal g2 = {\n")
    for gram in sorted(g2):
        idxs = ",".join(str(i + 1) for i in g2[gram])
        parts.append(f"  [{lua_str(gram)}] = {{{idxs}}},\n")
